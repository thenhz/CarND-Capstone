...
class Controller(object):
    ...
    def control(self, current_vel, dbw_enabled, linear_vel, angular_vel):
        ...
        if (current_time - self.log_time) > LOGGING_THROTTLE_FACTOR:
            ...
            rospy.loginfo("POSE: current_vel={:.2f}, linear_vel={:.2f}, vel_error={:.2f}".format(current_vel,
                                                                                                 linear_vel,
                                                                                                 vel_error))
            rospy.loginfo("POSE: throttle={:.2f}, brake={:.2f}, steering={:.2f}".format(throttle, brake, steering))
